...
- **세션 자동 유지**(계정별 토글) — 계정의 5시간 창이 리셋되는 즉시 창 없이 한마디를 보내 새 창을
  곧바로 시작시킵니다(트레이 앱이 상주 중일 때 동작). 계정마다 두 가지 방식: **항상**(24시간 내내 리셋 즉시
  재시작) 또는 **시간표** — 원하는 리셋 시각(예: *첫 리셋 11:00*)과 하루 창 개수를 정하면 5시간 전(06:00)에 첫
  창을 시작해 11:00 · 16:00 · 21:00 · 02:00 에 리셋되게 맞추고, 그 뒤엔 다음 날 첫 창까지 쉬어 매일 같은
  시간표를 유지합니다. 목록에서 방식과 다음 창 시작 예정을 바로 볼 수 있습니다.
- **계정 간 세션 이어하기** — 특정 계정의 지난 Claude Code 대화 세션을 훑어보고, *다른* 계정으로
  이어서 진행할 수 있습니다(사본으로 열려 원본은 그대로 보존). 계정을 오가며 이어해도 안전합니다: 대상 계정에
  같은 세션의 오래된 사본이 있으면 더 진행된 쪽으로 자동 교체(기존 사본은 백업)하고, 양쪽이 서로 다르게
  진행됐으면 어느 쪽을 쓸지 물어봅니다.
- **계정 상태줄** — 동시 실행 시 claude 하단에 계정 이메일 · 플랜 · 이름 · 실시간 세션% 를 표시.
- **프로필을 유지한 채 다시 로그인** — 플랜·구독이 바뀌었거나 토큰이 만료됐을 때 **다시 로그인**을 누르면
  기존 자격증명을 백업한 뒤 격리된 새 창이 **곧바로 로그인 화면**으로 열립니다. 토큰을 더 이상 갱신할 수 없는
  계정은 목록과 트레이 메뉴에 *활성* 대신 **"다시 로그인 필요"**로 표시됩니다.
- **실행 옵션**: **PowerShell / cmd** 선택, **관리자 권한으로 실행**(켜면 실행할 때마다 UAC 확인 창),
  `--dangerously-skip-permissions` 부여 여부 선택(*다시 묻지 않음*으로 선택값 기억 — **설정**에서 언제든 재설정).
  탐색기 우클릭 실행에도 적용됩니다.
